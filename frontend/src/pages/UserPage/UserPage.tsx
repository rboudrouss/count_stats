...
class UserPage extends React.Component {
  state: { loading: boolean, id: string, user: User | null, userMsg: Message[], highScore: { date: string, nb: number }, nbMsgs: number } = {
  ...
  async componentDidMount() {
    ...
    this.setState({ userMsg, user, highScore: { date: interMsg[0][0], nb: interMsg[0][1].length }, nbMsgs: nbMsgs ?? 0, loading: false });
  }


  render() {
    const { user, userMsg, highScore: { date, nb }, nbMsgs, loading } = this.state;
    return (
